...
# run_report_ranges.py previously replaced base._fix_html and then called
# base._fix_html from inside the replacement, causing infinite recursion.
_ORIGINAL_DEDUPE_FIX = base._fix_html


def _range_aware_fix_html(raw_html: bytes) -> tuple[bytes, int]:
    """Deduplicate positions, add range metrics, then inject range UI."""
    ...
    data["performanceMetricsByPeriod"] = ranges._period_metrics(data)
